...
function install_base_pkgs() {
    ...
    case "$usedSystem" in
        "ubuntu"|"debian")
            install_pkgs "$1" bind9 ccache curl freeradius git gnupg net-tools openssh-server socat tcpdump vim
            ;;
        "fedora"|"rhel")
    ...
    for node in $(seq 1 "$2"); do
        ...
        case "$usedSystem" in
            "ubuntu"|"debian")
                ...
                # so we need to build exactl list of packages to install
