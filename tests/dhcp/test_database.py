...
def test_db_retry_legallog_stop_retry_exit(backend, dhcp_version):
    ...
    srv_control.add_parameter_to_hook("libdhcp_legal_log.so", 'name', world.f_cfg.db_name)
    srv_control.add_parameter_to_hook("libdhcp_legal_log.so", 'password', world.f_cfg.db_passwd)
    srv_control.add_parameter_to_hook("libdhcp_legal_log.so", 'type', backend)
    srv_control.add_parameter_to_hook("libdhcp_legal_log.so", 'user', world.f_cfg.db_user)
    srv_control.add_parameter_to_hook("libdhcp_legal_log.so", 'retry-on-startup', True)
