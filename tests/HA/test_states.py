...
from HA.steps import send_cc, HOT_STANDBY, LOAD_BALANCING, wait_until_ha_state
...
@pytest.mark.v6
@pytest.mark.v4
def test_HA_state_hold_lb_always(dhcp_version):
    ...
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "waiting"
    assert send_cc(dhcp_version=dhcp_version, dest=world.f_cfg.mgmt_address_2)["arguments"]["state"] == "waiting"
    srv_msg.forge_sleep(10, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "waiting"
    assert send_cc(dhcp_version=dhcp_version, dest=world.f_cfg.mgmt_address_2)["arguments"]["state"] == "waiting"

    # continue server1 from WAITING
    assert send_cc(dhcp_version=dhcp_version, cmd={"command": "ha-continue"})["text"] == 'HA state machine continues.'
    wait_until_ha_state("syncing", dhcp_version=dhcp_version)
    srv_msg.forge_sleep(5, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "syncing"
    ...
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "ready"
    _send_message(dhcp=dhcp_version, expect_answer=False)
    ...
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "load-balancing"
    srv_msg.forge_sleep(5, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "load-balancing"
    ...
    srv_msg.forge_sleep(10, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "partner-down"
    srv_msg.forge_sleep(5, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "partner-down"
...
@pytest.mark.v6
@pytest.mark.v4
def test_HA_state_hold_lb_once(dhcp_version):
    ...
    srv_msg.forge_sleep(10, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "ready"
    assert send_cc(dhcp_version=dhcp_version, dest=world.f_cfg.mgmt_address_2)["arguments"]["state"] == "ready"
    _send_message(dhcp=dhcp_version, expect_answer=False)
    srv_msg.forge_sleep(3, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "ready"
    assert send_cc(dhcp_version=dhcp_version, dest=world.f_cfg.mgmt_address_2)["arguments"]["state"] == "ready"
    ...
    wait_until_ha_state("partner-down", dhcp_version=dhcp_version)
    assert send_cc(dhcp_version=dhcp_version, cmd={"command": "ha-continue"})["text"] == 'HA state machine is not paused.'
    srv_msg.forge_sleep(5, 'seconds')
...
@pytest.mark.v6
@pytest.mark.v4
def test_HA_state_hold_hs_once(dhcp_version):
    ...
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "hot-standby"
    srv_msg.forge_sleep(5, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "hot-standby"
...
@pytest.mark.v6
@pytest.mark.v4
def test_HA_state_hold_hs_always(dhcp_version):
    ...
    srv_msg.forge_sleep(5, 'seconds')
    assert send_cc(dhcp_version=dhcp_version)["arguments"]["state"] == "hot-standby"
    assert send_cc(dhcp_version=dhcp_version, dest=world.f_cfg.mgmt_address_2)["arguments"]["state"] == "hot-standby"
