...
Scenario: v6.host.reservation.cql.duplicate-reservation-duid
Test Setup:
Server is configured with 3000::/30 subnet with 3000::1-3000::10 pool.

Use Cassandra reservation system.

Create new Cassandra reservation identified by duid 00:03:00:01:f6:f5:f4:f3:f2:01.
Add dhcp6_subnet_id 1 to Cassandra reservation record id 1.
Add IPv6 address reservation 3000::1 with iaid $(EMPTY) to Cassandra record id 1.

Create new Cassandra reservation identified by duid 00:03:00:01:f6:f5:f4:f3:f2:01.
Add dhcp6_subnet_id 1 to Cassandra reservation record id 2.
Add IPv6 address reservation 3000::2 with iaid $(EMPTY) to Cassandra record id 2.
Upload hosts reservation to Cassandra database.
...
@v6 @host_reservation @kea_only
Scenario: v6.host.reservation.cql.duplicate-reservation-address
Test Setup:
Server is configured with 3000::/30 subnet with 3000::1-3000::10 pool.
...
Create new Cassandra reservation identified by duid 00:03:00:01:f6:f5:f4:f3:f2:11.
Add dhcp6_subnet_id 1 to Cassandra reservation record id 1.
Add IPv6 address reservation 3000::1 with iaid $(EMPTY) to Cassandra record id 1.
Upload hosts reservation to Cassandra database.
...
@v6 @host_reservation @kea_only 
  Scenario: v6.host.reservation.cql.conflicts-two-entries-for-one-host-different-subnets
  Test Setup:
  Server is configured with 3000::/30 subnet with 3000::1-3000::10 pool.
  Use Cassandra reservation system.
  ...
  Create new Cassandra reservation identified by hw-address f6:f5:f4:f3:f2:01.
  Add dhcp6_subnet_id 2 to Cassandra reservation record id 2.
  Add IPv6 address reservation 3000::3 with iaid $(EMPTY) to Cassandra record id 2.
...
@v6 @host_reservation @kea_only 
  Scenario: v6.host.reservation.cql.conflicts-reconfigure-server-with-reservation-of-used-address
  Test Setup:
  ...
  Server is configured with 3000::/30 subnet with 3000::1-3000::10 pool.
  Use Cassandra reservation system.
  Create new Cassandra reservation identified by hw-address 00:03:00:01:f6:f5:f4:f3:f2:01.
  Add dhcp6_subnet_id 1 to Cassandra reservation record id 1.
  Add IPv6 address reservation 3000::1 with iaid $(EMPTY) to Cassandra record id 1.
...
@v6 @host_reservation @kea_only 
  Scenario: v6.host.reservation.cql.conflicts-reconfigure-server-with-reservation-of-used-address-2
  Test Setup:
  Server is configured with 3000::/30 subnet with 3000::1-3000::2 pool.
  Send server configuration using SSH and config-file.
  DHCP server is started.
...
@v6 @host_reservation @kea_only
  Scenario: v6.host.reservation.cql.conflicts-reconfigure-server-with-reservation-of-used-address-renew-before-expire
  Test Setup:
  Server is configured with 3000::/30 subnet with 3000::1-3000::2 pool.
  ...
  Create new Cassandra reservation identified by duid 00:03:00:01:f6:f5:f4:f3:f2:01.
  Add dhcp6_subnet_id 1 to Cassandra reservation record id 1.
  Add IPv6 address reservation 3000::2 with iaid $(EMPTY) to Cassandra record id 1.
  Upload hosts reservation to Cassandra database.
...
@v6 @host_reservation @kea_only
  Scenario: v6.host.reservation.cql.conflicts-reconfigure-server-with-reservation-of-used-address-renew-after-expire
  Test Setup:
