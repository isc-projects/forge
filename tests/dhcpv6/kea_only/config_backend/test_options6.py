...
def test_options_pool():
    ...
    cfg.add_subnet(pool_options={"code": 22, "csv-format": True, "data": "2001::3",
                                 "name": "sip-server-addr", "space": "dhcp6"})
    get_address(req_opt=22, exp_option={"code": 22, "data": "2001::3"})


def test_options_subnet():
    cfg = setup_server_for_config_backend_cmds()

    cfg.add_subnet(subnet_options={"code": 22, "csv-format": True, "data": "2001::4",
                                   "name": "sip-server-addr", "space": "dhcp6"})

    get_address(req_opt=22, exp_option={"code": 22, "data": "2001::4"})


def test_options_network():
    cfg = setup_server_for_config_backend_cmds()
    network_cfg, _ = cfg.add_network(network_options={"code": 23, "csv-format": True, "data": "2001::3",
                                                      "name": "dns-servers", "space": "dhcp6"})
    cfg.add_subnet(network=network_cfg)

    get_address(req_opt=[23, 27], exp_option={"code": 23, "data": "2001::3"})
...
def test_options_global():
    ...
    get_address(req_opt=[23, 27], no_exp_option={"code": 23})


def test_options_all_levels():
    cfg = setup_server_for_config_backend_cmds()
    cfg.add_option(code=31, csv_format=True, data="2001::31", name="sntp-servers", space="dhcp6")
    network_cfg, _ = cfg.add_network(network_options={"code": 28, "csv-format": True, "data": "2001::32",
                                                      "name": "nisp-servers", "space": "dhcp6"})
    cfg.add_subnet(network=network_cfg,
                   subnet_options={"code": 27, "csv-format": True, "data": "2001::33",
                                   "name": "nis-servers", "space": "dhcp6"},
                   pool_options={"code": 22, "csv-format": True, "data": "2001::34",
                                 "name": "sip-server-addr", "space": "dhcp6"})

    get_address(req_opt=[31, 27, 22, 28], exp_option=[{"code": 31, "data": "2001::31"},
...
def test_options_inherit():
    ...
    get_address(req_opt=23, exp_option={"code": 23, "data": "2001::31"})

    # del subnet, set network with option and with subnet without option
    cfg.del_subnet()
    network_cfg, _ = cfg.add_network(network_options={"code": 23, "csv-format": True, "data": "2001::32",
                                                      "name": "dns-servers", "space": "dhcp6"})
    cfg.add_subnet(network=network_cfg)

    get_address(req_opt=23, exp_option={"code": 23, "data": "2001::32"})

    # delete subnet without option, set with new subnet with option - options on higher level are
    # still configured
    cfg.del_subnet()
    cfg.add_subnet(network=network_cfg,
                   subnet_options={"code": 23, "csv-format": True, "data": "2001::33",
                                   "name": "dns-servers", "space": "dhcp6"})

    get_address(req_opt=23, exp_option={"code": 23, "data": "2001::33"})
    ...
    cfg.add_subnet(network=network_cfg,
                   subnet_options={"code": 23, "csv-format": True, "data": "2001::33",
                                   "name": "dns-servers", "space": "dhcp6"},
                   pool_options={"code": 23, "csv-format": True, "data": "2001::34",
                                 "name": "dns-servers", "space": "dhcp6"})

    get_address(req_opt=23, exp_option={"code": 23, "data": "2001::34"})
