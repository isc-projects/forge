...
def test_server_tag_subnet():
    ...
    cfg_xys.add_subnet(server_tags=["abc"], subnet="2001:db8:1::/64", id=1,
                       pools=[{'pool': "2001:db8:1::1-2001:db8:1::1"}])

    get_rejected()

    cfg_xys.add_subnet(server_tags=["unassigned"], subnet="2001:db8:1::/64", id=3,
                       pools=[{'pool': "2001:db8:1::10-2001:db8:1::10"}])

    get_rejected()

    # add configuration for xys, check result
    cfg_xys.add_subnet(server_tags=["xys"], subnet="2001:db8:1::/64", id=2,
    ...
    cfg_qwe.add_subnet(server_tags=["all"], subnet="2001:db8:1::/64", id=2,
...
def test_server_tag_global_option():
    ...
    cfg_xyz.add_subnet(server_tag="xyz")
...
def test_server_tag_global_parameter():
    ...
    cfg.set_global_parameter(server_tags=["unassigned"], valid_lifetime=5000)
    get_address(exp_ia_na_iaaddr_validlft=7200)
    cfg.set_global_parameter(server_tags=["xyz"], valid_lifetime=5001)
    cfg.set_global_parameter(server_tags=["all"], valid_lifetime=5002)
