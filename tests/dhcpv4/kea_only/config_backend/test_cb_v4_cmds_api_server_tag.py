...
def test_remote_server_tag_set(backend):
    ...
    _set_server_tag(backend=backend)
...
def test_remote_server_tag_get(backend):
    _setup_server(backend)
    _set_server_tag(backend=backend)
    cmd = dict(command="remote-server4-get", arguments={"remote": {"type": backend},
...
def test_remote_server_tag_get_all(backend):
    _setup_server(backend)
    _set_server_tag(backend=backend)
    _set_server_tag(backend=backend, tag="xyz")
    cmd = dict(command="remote-server4-get-all", arguments={"remote": {"type": backend}})
    response = srv_msg.send_ctrl_cmd(cmd)
...
def _add_server_tag(backend=None, server_tag=None):
    cmd = dict(command="remote-server4-set", arguments={"remote": {"type": backend},
                                                        "servers": [{"server-tag": server_tag}]})
    srv_msg.send_ctrl_cmd(cmd)


def _subnet_set(server_tags, subnet_id, pool, backend=None, exp_result=0, subnet="192.168.50.0/24", ):
...
def _subnet_list(command, server_tags, backend=None, exp_result=0):
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_subnet4_server_tags_delete_server_tag_keep_data(backend):
    _setup_server(backend)
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_subnet4_get_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_subnet4_get_server_tags_all_incorrect_setup(backend):
    _setup_server(backend)
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_subnet4_del_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _subnet_set(backend=backend, server_tags=["abc"], subnet_id=5, pool="192.168.50.1-192.168.50.100")
...
def _network_set(server_tags, backend, network_name="florX", exp_result=0):
    cmd = dict(command="remote-network4-set", arguments={"remote": {"type": backend}, "server-tags": server_tags,
...
def _network_get(command, backend, exp_result=0, network_parameter=None):
    cmd = dict(command=command, arguments={"remote": {"type": backend}})
...
def _network_list(command, backend, server_tags, exp_result=0):
    cmd = dict(command=command, arguments={"remote": {"type": backend}, "server-tags": server_tags})
...
def _network_del(backend, exp_result=0, network_parameter=None):
    return _network_get(backend=backend, command="remote-network4-del",
                        exp_result=exp_result, network_parameter=network_parameter)
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_network4_server_tags_remove_server_tag_keep_data(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _network_set(backend=backend, server_tags=["abc"])
    _network_set(backend=backend, server_tags=["xyz"], network_name="flor1")
    _network_set(backend=backend, server_tags=["all"], network_name="top_flor")

    resp = _network_get(backend=backend, command="remote-network4-get", network_parameter={"name": "florX"})
    _network_check_res(resp, server_tags=["abc"], network_name="florX")

    resp = _network_get(backend=backend, command="remote-network4-get", network_parameter={"name": "flor1"})
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_network4_get_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _network_set(backend=backend, server_tags=["abc"])
    ...
    resp = _network_list(backend=backend, command="remote-network4-list", server_tags=["xyz"])
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_network4_get_server_tags_all_incorrect_setup(backend):
    _setup_server(backend)
    ...
    _add_server_tag(backend=backend, server_tag="xyz")
    _network_set(backend=backend, server_tags=["abc"])
    _network_set(backend=backend, server_tags=["xyz"])
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_network4_del_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _network_set(backend=backend, server_tags=["abc"])
    ...
    resp = _network_get(backend=backend, command="remote-network4-get", network_parameter={"name": "top_flor"})
...
def _option_set(server_tags, backend, exp_result=0, code=3, opt_data="1.1.1.1"):
...
def _option_del(server_tags, backend, exp_result=0, opt_code=None):
    return _option_get(backend=backend, command="remote-option4-global-del", server_tags=server_tags,
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_option4_get_server_tags_all(backend):
    _setup_server(backend)
    ...
    resp = _option_get(backend=backend, command="remote-option4-global-get-all", server_tags=["abc"], opt_code=3)
...
@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_option4_get_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _option_set(backend=backend, server_tags=["abc"])
    ...
    resp = _option_get(backend=backend, command="remote-option4-global-get-all", server_tags=["xyz"], opt_code=3)
    _check_option_result(resp, count=1, server_tags=["xyz"], opt_name="routers", opt_data="2.2.2.2")


@pytest.mark.parametrize('backend', ['mysql', 'postgresql'])
def test_remote_option4_del_server_tags(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _add_server_tag(backend=backend, server_tag="xyz")
    _option_set(backend=backend, server_tags=["abc"])
...
def _optdef_set(server_tags, backend, exp_result=0, opt_code=222, opt_name="foo", opt_type="uint32"):
...
def _optdef_get(command, server_tags, backend, exp_result=0, option_def_parameter=None):
    cmd = dict(command=command, arguments={"remote": {"type": backend},
                                           "server-tags": server_tags})
...
def _optdef_del(server_tags, backend, exp_result=0, option_def_parameter=None):
    return _optdef_get("remote-option-def4-del", server_tags, backend, exp_result=exp_result,
...
def test_remote_option_def_get_server_tags_tmp(backend):
    ...
    # simple test for one ticket https://gitlab.isc.org/isc-projects/kea/issues/737
    _add_server_tag(backend=backend, server_tag="abc")
    _optdef_set(backend=backend, server_tags=["all"])
    resp = _optdef_get(backend=backend, command="remote-option-def4-get-all", server_tags=["abc"])
...
def test_remote_option_def_get_server_tags_get_non_existing_tag(backend):
    ...
    resp = _optdef_get(backend=backend, command="remote-option-def4-get", server_tags=["abc"],
...
def test_remote_option_def_remove_server_tag_and_data(backend):
    _setup_server(backend)
    _add_server_tag(backend=backend, server_tag="abc")
    _optdef_set(backend=backend, server_tags=["abc"])
    ...
    _optdef_get(backend=backend, command="remote-option-def4-get", server_tags=["abc"],
...
def test_remote_option_def_get_server_tags(backend):
    ...
    # server tag "all"
    _optdef_set(backend=backend, server_tags=["abc"])
    _optdef_set(backend=backend, server_tags=["xyz"], opt_type="string")
    _optdef_set(backend=backend, server_tags=["all"], opt_code=233, opt_name="bar")

    resp = _optdef_get(backend=backend, command="remote-option-def4-get", server_tags=["abc"],
                       option_def_parameter={"code": 222})
    _check_optdef_result(resp, server_tags=["abc"])

    resp = _optdef_get(backend=backend, command="remote-option-def4-get", server_tags=["xyz"],
                       option_def_parameter={"code": 222})
    _check_optdef_result(resp, server_tags=["xyz"], opt_type="string")

    # this one should fail
    resp = _optdef_get(backend=backend, command="remote-option-def4-get", server_tags=["all"],
                       option_def_parameter={"code": 222}, exp_result=3)
    assert resp["arguments"]["count"] == 0
    assert resp["result"] == 3
    ...
    resp = _optdef_get(backend=backend, command="remote-option-def4-get-all", server_tags=["xyz"])
    _check_optdef_result(resp, server_tags=["xyz"], count=2, opt_type="string")
    assert resp["arguments"]["option-defs"][1]["metadata"] == {"server-tags": ["all"]}
    assert resp["arguments"]["option-defs"][1]["name"] == "bar"
    assert resp["arguments"]["option-defs"][1]["code"] == 233

    resp = _optdef_get(backend=backend, command="remote-option-def4-get-all", server_tags=["all"])
    _check_optdef_result(resp, server_tags=["all"], opt_code=233, opt_name="bar")
...
def test_remote_option_def_del_server_tags(backend):
    ...
    _optdef_set(backend=backend, server_tags=["abc"])
    _optdef_set(backend=backend, server_tags=["xyz"], opt_type="string")
    _optdef_set(backend=backend, server_tags=["all"], opt_type="uint8")
    ...
    _optdef_del(["all"], backend, option_def_parameter={"code": 222})
    ...
    _optdef_del(["abc"], backend, option_def_parameter={"code": 222})
