...
@v6 @controlchannel
  Scenario: control.channel.http.set-config-basic
  Test Setup:
  ...
  Using existing HTTP $(SRV4_ADDR):8000 connection send: {"command": "set-config","arguments":  $(SERVER_CONFIG) }
...
@v6 @controlchannel
  ...
  Test Setup:
  Server is configured with 2001:db8:1::/64 subnet with 2001:db8:1::1-2001:db8:1::1 pool.
  Server has control agent configred on HTTP connection with address $(SRV4_ADDR):8000 and socket UNIX path: $(SOFTWARE_INSTALL_DIR)var/kea/control_socket2.
  Generate server configuration file.
...
@v6 @controlchannel
  ...
  Using existing HTTP $(SRV4_ADDR):8000 connection send: {"command": "get-config","arguments": {} }
...
@v6 @controlchannel
  ...
  Server has control agent configred on HTTP connection with address $(SRV4_ADDR):8000 and socket UNIX path: $(SOFTWARE_INSTALL_DIR)var/kea/control_socket_ANOTHER_ONE.
  ...
  Using existing HTTP $(SRV4_ADDR):8000 connection send: {"command": "test-config","arguments":  $(SERVER_CONFIG) }
  #should be ok
...
@v6 @controlchannel
Scenario: control.channel.http.write-config
  ...
  Pause the Test.
  Using existing HTTP $(SRV4_ADDR):8000 connection send: {"command": "write-config","arguments": {} } #TODO probably confing file location/name
