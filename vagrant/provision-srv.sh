#!/bin/bash
set -e -x

if [ ! -f $HOME/.ssh/authorized_keys.orig ]; then
    cp $HOME/.ssh/authorized_keys $HOME/.ssh/authorized_keys.orig
fi
cp $HOME/.ssh/authorized_keys.orig $HOME/.ssh/authorized_keys
cat $HOME/.ssh/id_rsa.pub >> $HOME/.ssh/authorized_keys
echo "ssh-rsa  godfryd@u-1810-kea-cli" >> $HOME/.ssh/authorized_keys

sudo apt update
sudo apt install -y socat

if ! ifconfig | grep '2001:db8:0:f101::1'; then
    sudo ip -6 addr add 2001:db8:0:f101::1/64 dev enp0s9
fi
if ! ifconfig | grep '2001:db8:1:f101::1'; then
    sudo ip -6 addr add 2001:db8:1:f101::1/64 dev enp0s10
fi
